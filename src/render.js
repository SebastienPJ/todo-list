...
const renderTodo = (() => {
  ...
  const stopScrolling = () => {
    document.classList.toggle('stop-scrolling');
  };
  ...
  const prefillEditForm = (e) => {
    const _todoIndex = e.target.dataset.overallTaskIndex;

    const _allTodoArray = todo.getTodoList();

    const _currentTodoObject = _allTodoArray[_todoIndex];
  ...
  const prefillProjForm = (values, optionsArray) => {
    const _currentProjName = values.listOfProjects[values.projectIndex];

    _editProjName.value = _currentProjName;

    const _selectedValues = values.tasksBelongingToProj.map((task) => task.title);
